			...
			if(xmlhttp.status == 200) {			// And we have text,
				...
				if (method == 'user.getrecenttracks') {
					...
					for (i = 0; i < number; i++) {
						...
						// console.log(artist);
						element.innerHTML += "<a href='" + artist.url + "' target='_blank' rel='noreferrer noopener'><img src='" + artist.image[1]["#text"] + "' title='" + artist.name + ' - ' + artist.playcount + " plays' alt='" + artist.name + ' - ' + artist.playcount + " plays';/>";
					}
				}
			 }
		}
